$(document).ready(function() {
	...
	function loadPantryFromFirebase(childSnapshot) {
		...
		$("#pantry-list").append("<tr data-name=\"" + childSnapshot.key + "\"><td>" + childSnapshot.val().ingredient + "</td><td>" + childSnapshot.val().quantity + "</td><td>" + childSnapshot.val().unit + "</td><td>" + "<input type='checkbox' id='check" + check + "' class='pantryCheckBox'/><label for='check" + check + "'></label></td><td>" + "<a target='_blank' class='waves-effect waves-light btn blue darken-1 modal-trigger amazonButton' onclick='window.open(\"https://www.amazon.com/s/ref=nb_sb_noss_2?url=srs%3D7301146011%26search-alias%3Dpantry&field-keywords=" + childSnapshot.val().ingredient + "\")' href='#amazon-modal' href=\'php/indexer.php?Keywords=" + childSnapshot.val().ingredient + "'\ id='findOnAmazon'>Amazon</a></td><td>" + "<a class='btn-floating btn red box'><i class='small material-icons deletePantryItemButton'>delete_forever</i></a></td></tr>");
		check++;
